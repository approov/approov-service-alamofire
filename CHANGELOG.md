...
### Added
- Manual release tagging via the `release` job in `build_and_test.yml` (triggered from the Actions tab against `main` with `workflow_dispatch`; gated on `build-and-test`), with a companion `verify-release` job that runs on every push/PR to assert the `dev` placeholders and a CHANGELOG version header are present. On release the top CHANGELOG entry drives the git tag, and the version is stamped in lock-step into **both** package manifests — the SwiftPM `Package.swift` (`releaseTAG`) and the CocoaPods `ApproovSession.podspec` (`s.version`) — plus the runtime user-property string, before the release commit is tagged. `main` keeps the `dev` placeholders; each tag carries the stamped version. Re-tagging is skipped when the tag already exists.
